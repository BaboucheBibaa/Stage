...
def boucle_affichage(file: GestionSorties) -> None:
    """
    Seul endroit du programme autorisé à faire print().
    Tourne dans son propre thread et vide la queue au fil de l'eau.
    S'arrête dès que file.stop() enfile la sentinelle None.
    """
    while True:
        item: MessageAffichage | None = file.get()
...
def boucle_chat(dm: DialogueModule, file: GestionSorties) -> None:
    """
    ...
    while True:
        try:
            ...
            file.enqueue("Au revoir !", source="dialogue")
            break
        ...
        file.enqueue(reponse, source="dialogue")
...
def main() -> None:
    ...
    schedule = DeclenchementProactivite(llm, ID_PROFIL, gestionnaire_sortie=file)
    schedule.start()
